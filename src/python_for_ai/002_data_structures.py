...
print(weather_reading.get("humidity"))  # .get() -- returns None instead of crashing if missing || IMP
weather_reading["humidity"] = 60        # adding a new key
print(weather_reading)

for key, value in weather_reading.items():   # looping over BOTH keys and values || IMP
    print(f"{key}: {value}")
...
warm_cities = [r["city"] for r in readings if r["temperature_c"] > 20]   # a list comprehension || IMP
